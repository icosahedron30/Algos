...
#define linf 1e18+1
using namespace std;
//PERSISTENT segment tree for sum
struct Vertex{
    int sum=0;
    Vertex *l,*r;
...
void solve(){
    ...
    while(q--){
        int ind;cin>>ind;
        switch(ind){
            case 1:int l,r,ind;cin>>l>>r>>ind;cout<<get(roots[ind],l,r,1,n)<<"\n";break;
            case 2:int pos,nw;cin>>pos>>nw;roots.pb(update(roots[roots.size()-1],1,n,pos,nw));break;
            default:break;
        }
    }
}
